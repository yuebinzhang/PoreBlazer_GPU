...
subroutine lattice_calculations_gpu
    ...
    !$acc&   lattice_space_n, lattice_lj_he) &
    !$acc& delete(coords_temp, asigma2, asigma2_he, aeps_he, asigma, asigma2_n, &
    !$acc&   atype, ell, hicut2, cube_size, ncubesx, ncubesy, ncubesz, natoms)
...
subroutine surface_area_gpu
    ...
    do i=1, natoms
        seed = mod(1_8 + i * 694847539_8, 2147483647_8); ncount_arr(i) = 0
        do j=1, nsample
    ...
    !$acc& delete(coords, asigma_n, asigma2_n, atype, ell, lattice_space_n, &
    !$acc&   vlist, nneighbors, max_neighbors)
...
subroutine pore_distribution_gpu
    ...
    integer, allocatable :: lcl_n_cubes(:)
    ...
    integer*8 :: seed, new_seed
    integer*8 :: t1, t2, t_rate, t_max; real*8 :: elapsed
    real*8, allocatable :: trial_xc(:), trial_yc(:), trial_zc(:)
    ...
    allocate(lcl_PA1(nc_local)); lcl_PA1(1:nc_local) = PA1(1:nc_local)
    allocate(lcl_PA2(nc_local)); lcl_PA2(1:nc_local) = PA2(1:nc_local)
    allocate(lcl_PA3(nc_local)); lcl_PA3(1:nc_local) = PA3(1:nc_local)
    allocate(lcl_PA4(nc_local)); lcl_PA4(1:nc_local) = PA4(1:nc_local)
    if(property == "Network-accessible ") then
        allocate(lcl_n_cubes(nn_cubes)); lcl_n_cubes(1:nn_cubes) = n_cubes(1:nn_cubes)
    end if
    ...
    ! Total PSD branch
    if(property == "Total ") then
        ! Precompute trial points on CPU to avoid nvfortran LCG+large-array bug
        ...
        !$acc enter data copyin(lcl_PA1, lcl_PA2, lcl_PA3, lcl_PA4, lcl_psd_cumul)
        !$acc enter data copyin(trial_xc, trial_yc, trial_zc)
        !$acc parallel loop gang &
        !$acc& private(m, nx1, ny1, nz1, xc, yc, zc, xc1, yc1, zc1, &
        !$acc&   d1, d2, d3, rdist2, sigma2_ref, sigma_ref, bin) &
        !$acc& firstprivate(ncycles, nc_local, nbins, ncubesx, ncubesy, ncubesz, &
        !$acc&   cube_size, ellx, elly, ellz) &
        !$acc& present(lcl_PA1, lcl_PA2, lcl_PA3, lcl_PA4, lcl_psd_cumul, &
        !$acc&   trial_xc, trial_yc, trial_zc)
        do i = 1, ncycles
        ...
        !$acc exit data delete(lcl_PA1, lcl_PA2, lcl_PA3, lcl_PA4, lcl_psd_cumul, &
        !$acc&   trial_xc, trial_yc, trial_zc)
        deallocate(trial_xc, trial_yc, trial_zc)
