...
module parameters
        ...
        integer :: ncycles_psd                           ! number of Monte Carlo trials for PSD (default 10000)
...
subroutine initialize(filename)
   ...
   atvec1 = fundcell_unslant(fcell, atvec1)
    ...
    read(3,*) vis_option
    read(3,*,err=425,end=425) ncycles_psd
    go to 426
425 ncycles_psd = 10000    ! default if not specified
426 continue
...
subroutine pore_distribution
    ...
    implicit none
    type(vectype)                         :: atvec1, atvec2, sepvec
    integer                               :: i, j, k, l, nx, ny, nz, nx1, ny1, nz1, icount, m, bin, isite, ncycles, ivis, nc_local
    ...
    ! here we go through all cubelets accessible to nitrogen (or point for Total PSD),
    ! store distances between the centers of cubelets and the surface of the nearest
    ! neighbour atom in PA1 array, and sort PA1 in an ascending order
    ! For Total PSD: use lattice_space (point-accessible, geometry-only, probe-independent)
    ! For Network PSD: use lattice_space_n (N2-accessible, probe-dependent)

    if(property == "Total ") then
        !--- Total PSD: geometry-based, filter by point-accessible (probe-independent)
        do l=1, ncubesz
            do k=1, ncubesy
                do j=1, ncubesx
        ...
        !--- Network-accessible PSD: N2-probe-dependent
        do l=1, ncubesz
            do k=1, ncubesy
                do j=1, ncubesx
    ...
    sigma2_ref=0.0
        ...
        if(property == "Total ") then
            ! Total PSD: sample from point-accessible cubelets
            ...
            ! Network-accessible PSD: sample from N2-accessible cubelets
            if(nn_cubes == 0) cycle
            isite = int(rranf()*dble(nn_cubes)) + 1
        ...
        atvec1%comp(1) = dble(nx-1)*cube_size+0.5*cube_size
        atvec1%comp(2) = dble(ny-1)*cube_size+0.5*cube_size
        atvec1%comp(3) = dble(nz-1)*cube_size+0.5*cube_size

        ! Number of PA entries to search: matches the PA fill above
        if(property == "Total ") then
            nc_local = ng_cubes
        else
            nc_local = nn_cubes
        ...
        ! PA search: from LARGEST to SMALLEST lattice_rdist2,
        ! find the first cubelet whose safe sphere encloses the test point.
        ! This gives the LARGEST sphere radius that fits at the test point.
        do j=nc_local, 1, -1
            nx1 = PA2(j)
            ny1 = PA3(j)
            nz1 = PA4(j)
        ...
        sigma_ref = sqrt(sigma2_ref)
        bin=int(2.0*sigma_ref/binsize)+1
